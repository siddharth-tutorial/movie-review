...
// import { Breadcrumb, Button, Card, CardBody, CardSubtitle, CardTitle, Col, Container, Row } from 'react-bootstrap'
...
//               `https://www.omdbapi.com/?s=${query}&type=series&apikey=${'apfd765352iKey'}`
//             );
...
//             <Breadcrumb.Item active className="text-white">TV Shows</Breadcrumb.Item>
//           </Breadcrumb>

//              <h1 className="text-white text-center mb-4">📺 Explore TV Shows</h1>
...
//                         variant="warning"
//                         onClick={() =>
//                           navigate(`/tv-detail/${item.imdbID}`, { state: item })
//                         }
//                       >
//                         View Details
//                       </Button>
...
import "react-loading-skeleton/dist/skeleton.css";
import "animate.css";
import "./Movie.css"; // Reuse same styles
import Scrolltotop from "./Scrolltotop";
    ...
    const fetchAllShows = async () => {
      try {
        const results = await Promise.all(
          searchQueries.map(async (query) => {
            const res = await fetch(
              `https://www.omdbapi.com/?s=${query}&type=series&apikey=${apiKey}`
            );
            const data = await res.json();
            return data.Search || [];
          })
